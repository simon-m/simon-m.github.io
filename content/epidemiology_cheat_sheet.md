...
# TODO:
instead of an *event*, consider a *period diseased* which greatly help to show the difference between incidence and prevalence.
...
Variables \\(a\\) and \\(b\\) are absolute freuencies (counts).
Variable \\(p\\) and \\(q\\)are proportions.

- **Ratio**: \\(r = (a / b\\)
- **Proportion**: ratio when \\(a\\) is counted in b (e.g. \\(b = a + c \\)), relative frequency
- **Rate**: ratio or proportion within a specified time period (same for the numerator and denominator)
- **Odds**: \\(o = p / (1 - p)\\)
...
In general the *exposure time* (or exposed time, also person-time) is, i.e. the amount of time
...
**Closed population or cohort**: fixed number of individuals at start, except for loss to
...
## Incidence rate
Consider an *event* to be for instance a new diagnosis for a pathology of interest. Then, the incidence rate
...
**Remark**: the IR depends on the unit, which should be reported. It is not a proportion.
...
magnitude and independent of the diseased state.
...
Also called prevalence rate (although not a rate) or absolute risk.
...
Let \\((N_T\\) be the population size during period \\(T\\), assuming it remains fixed.
The *period prevalence* period \\(T\\) is:
\\[ \\text{PR}_{[-]} = \\frac{ \\text{#cases with disease during t}}{N_T} \\]

**Remark**: for the period prevalence, the denominator is the average population size during
the time interval.
...
Incidence is a *rate*, i.e. it gives information on the rate of occurrence of the event.
This can be seen as the denominator has a time dimension.

Prevalence, on the other hand is a proportion of people who got the event, whether it is
...
For rare diseases and a closed, or open *stationary* population, we have:
\\[ \text{PR}_{[-]} = IR \\cdot \\text{average disease duration} \\]


# Measures of association
Used to quantify the association between a factor (e.g. higher education) and the occurence of a
condition (e.g. myopia).
...
\\[O_1 = \\frac{P(D|E)}{P(~D|E)}\\]

Odds of having the disease in the non-exposed group:
\\[O_0 = \\frac{c/(c+d)}{d/c+d} = \\frac{c}{d}\\]
In terms of probabilities:
\\[O_0 = \\frac{P(D|~E)}{P(~D|~E)}\\]

Odds ratio:
\\[\\text{OR} = O_1 / O_0 = \\frac{ad}{bc}\\]
In terms of probabilities:
\\[\\text{OR} = \\frac{P(D|E)}{P(~D|E)} \\frac{P(~D|~E)}{P(D|~E)} \\]

## Odds ratio versus relative risk
Relative risk can only be calculated for cohort studies (see below) whereas odds ratio can be calculated
...
1. Build a *reference population* for the population under study. Both populations should be similar enough.
   ...
   thereof) must be available.
2. Using the stratum-specific proportions or rates from the reference population and the stratum sizes in the population
...
In other words, we need to know the prevalence  which is only known for cohort trials since disease status<is not used for defining the groups as opposed to case-control trials.
