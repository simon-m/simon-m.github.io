...
I will almost exclusively focus on *linear* models but be aware that teir exist generalized linear mixed models (GLMM) which are the mixed-model version of generalized linear models (GLM). Moreover, generalized stimating equations are an extension of GLM for correlated responses, and as such they can handle the same kind of dependences between covariates and response through the link function.
...
# Fixed and random effects: an example
...
One may also want to to control for the center (A or B) in which the measurement were made (in a
...
In a more general setting, random effects can be either categorical or continuous.


# Linear mixed effects models (LMM):
Linar mixed model include both fixed and random effects which are described separately.
...
As for the fixed effects model, given and \\(\\mathbf{U_{i}} = (U_{i1}, \\dots, U_{iq})\\) and
\\(\\mathbf{Z_{ij}} = (Z_{ij1}, \\dots, Z_{1jq})^T\\), this is equivalent to:
\\[ Y_{ij} = \\mathbf{\\beta} \\mathbf{x_{ij}} + \\mathbf{U_{i}} \\mathbf{Z_{ij}} + \\epsilon_{ij}\\]

Note that this is essentially the fixed-effects models to which were added with the random effects
\\(\\mathbf{Z_{ij}}\\) and the corresponding group-specific coefficients \\(\mathbf{U_{i}}\\).
...
Finally, letting \\(n = \\sum_{i} n_i\\) and \\(q = \\sum_{i} q_i\\) we have
...
## Variance components
As opposed to the fixed effects model, in this setting \\(\\epsilon\\) is no longer the only contributor to
the variance of \\(\\mathbf{Y}\\).
...
random variables which must be predicted.

I'll first assume that the covariance matrices are known to estimate \\(\\mathbf{\\beta}\\) and \\(\\mathbf{U}\\),
...
residual/restricted maximum likelihood (RML / REML).

## Random and fixed effects estimation
Let us start by assuming that \\(V = ZRZ^T + \\Sigma\\) is known, implying that  \\(R\\) and
\\(\\Sigma\\) are known too, and seek to estimate \\(\\mathbf{\\beta}\\) and \\(\\mathbf{U}\\).
One way is to solve Henderson's equations also called *mixed model equations* with the
following solutions:
...
Here *best* refers to the fact that it os the most *efficient*, i.e. has the minimum variance, among
...
The experimental design can sometimes give some information about the structure of these matrices and
one can specify their *structure* prior to estimation.
...
supported by statistical softwares, which is most of the time enough for any type of analysis.
...
\\(l(\\mathbf{Y}; \\mathbf{\\hat{\\beta}}(\\theta), V(\\theta))\\) with respect to \\(\\theta\\).

The main caveat of the ML estimation is that the variance estimates are negatively (downward) biased
...
The core idea is to perform MLE on a modified version of the data, namely linear combinations of \\(\\mathbf{Y}\\).
...
fitting the fixed effects ignoring the variance parameters.

In contrast with ML, the REML estimates for fixed effect are biased and those for random effect
are unbiased.

Remark: REML can also be described from a marginal likelihood point of vue. The marginal likelihood
...
Then log of the marginal likelihood is then maximized.

In practice, ML and REML are typically computed using iterative schemes such as the Newton-Raphson
algorithm and variants thereof, or the expectation-maximization algorithm.
...
For instance, linear regression consists of a normally distributed \\(Y\\) with a identity
...
the model.
Note that no assumption is made about the distribution of \\(Y\\) as opposed to LMM.
...
The estimation of the variance of \\(\\beta\\) on the other hand uses the so-called
*robust sandwich estimator* (some nice and short explanations in the context of OLS regression
...
# Mixed-effect models versus maringla models
...
can also handle data missing at random (MAR, randomness with dependence on the covariates).

Choosing between miwed models and marginal models is therefore primarily a question of scope (group-specific
versus population-wise) and also depend on what is known about the data (how certain are we about the correlation
structure? and about the complete randomness of missing data?).
